...
async function main() {
  ...
  for (const [density, px] of Object.entries(LEGACY)) {
    ...
    writeFileSync(path.join(dir, 'ic_launcher_round.png'), await circleMask(square, px));
    const fg = await onField(cat, ADAPTIVE[density], 0.66, { r: 0, g: 0, b: 0, alpha: 0 });
...
`;
  ...
  console.log('OK  launcher icons generated from brand/logo.png (rebuild to see them).');
}
